...
class MNN(nn.Module):
	...
	def _make_layer(self, supergroup, input_channel):
		...
		if(supergroup == "root"):
			...
			layers.append(InvertedResidual(ch_in=64, ch_out=64, width_multiplier=4, stride=1))
			layers.append(InvertedResidual(ch_in=64, ch_out=92, width_multiplier=2, stride=1))
			layers.append(InvertedResidual(ch_in=92, ch_out=92, width_multiplier=4, stride=1))
			layers.append(InvertedResidual(ch_in=92, ch_out=92, width_multiplier=4, stride=1))
			layers.append(InvertedResidual(ch_in=92, ch_out=128, width_multiplier=2, stride=1))

		elif(supergroup == "sg7"):
			layers.append(nn.Conv2d(in_channels=input_channel, out_channels=330, kernel_size=3, stride=2, padding=1))	
			layers.append(nn.Conv2d(in_channels=330, out_channels=330, kernel_size=3, stride=1, padding=1))
			layers.append(nn.Conv2d(in_channels=330, out_channels=330, kernel_size=3, stride=1, padding=1))
			layers.append(nn.Conv2d(in_channels=330, out_channels=330, kernel_size=3, stride=1, padding=1))
			layers.append(InvertedResidual(ch_in=330, ch_out=420, width_multiplier=6, stride=1))
			layers.append(InvertedResidual(ch_in=420, ch_out=420, width_multiplier=6, stride=1))
			layers.append(InvertedResidual(ch_in=420, ch_out=420, width_multiplier=4, stride=1))
			layers.append(InvertedResidual(ch_in=420, ch_out=250, width_multiplier=4, stride=1))
			...
			layers.append(nn.ReLU6(inplace=True))
			###
			layers.append(InvertedResidual(ch_in=128, ch_out=128, width_multiplier=6, stride=1))
			layers.append(InvertedResidual(ch_in=128, ch_out=128, width_multiplier=6, stride=1))
			layers.append(InvertedResidual(ch_in=128, ch_out=128, width_multiplier=4, stride=1))
			layers.append(InvertedResidual(ch_in=128, ch_out=250, width_multiplier=4, stride=1))
			layers.append(InvertedResidual(ch_in=250, ch_out=250, width_multiplier=6, stride=1))
			layers.append(InvertedResidual(ch_in=250, ch_out=250, width_multiplier=6, stride=1))
			layers.append(InvertedResidual(ch_in=250, ch_out=330, width_multiplier=4, stride=1))
			layers.append(InvertedResidual(ch_in=330, ch_out=330, width_multiplier=4, stride=1))
