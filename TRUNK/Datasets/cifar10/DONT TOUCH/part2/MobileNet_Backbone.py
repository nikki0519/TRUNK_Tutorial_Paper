...
class MNN(nn.Module):
	...
	def _make_layer(self, input_channel):
		...
		if(self.supergroup == "root"):
		...
		elif(self.supergroup == "sg2" or self.supergroup == "sg3"):
			layers.append(nn.Conv2d(in_channels=input_channel, out_channels=32, kernel_size=3, stride=2, padding=1))
			layers.append(nn.BatchNorm2d(num_features=32))
			layers.append(nn.ReLU6(inplace=True))
		...
		elif(self.supergroup == "sg1"):
			layers.append(nn.Conv2d(in_channels=input_channel, out_channels=32, kernel_size=3, stride=1, padding=1))
			layers.append(nn.BatchNorm2d(num_features=32))
			layers.append(nn.ReLU6(inplace=True))
			layers.append(nn.Dropout(p=0.5))
			###
			...
			layers.append(nn.ReLU6(inplace=True))
			layers.append(nn.Dropout2d(float=0.5, inplace=True))
			###
			layers.append(nn.Conv2d(in_channels=256, out_channels=512, kernel_size=3, stride=1, padding=1))
			layers.append(nn.Conv2d(in_channels=512, out_channels=512, kernel_size=5, stride=1, padding=1))
			layers.append(nn.ReLU6(inplace=True))
			layers.append(nn.Dropout2d(float=0.5, inplace=True))
			###
			layers.append(InvertedResidual(ch_in=512, ch_out=256, width_multiplier=2, stride=1))
			layers.append(InvertedResidual(ch_in=256, ch_out=256, width_multiplier=4, stride=1))
			layers.append(InvertedResidual(ch_in=256, ch_out=128, width_multiplier=8, stride=1))
			layers.append(nn.Dropout2d(float=0.3, inplace=True))
			layers.append(InvertedResidual(ch_in=128, ch_out=64, width_multiplier=16, stride=1))
